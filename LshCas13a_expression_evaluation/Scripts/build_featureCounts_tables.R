...
names(EcC3000_featureCounts_raw) <- sub("_S[0-9][0-9]_sorted.bam", "", names(EcC3000_featureCounts_raw))
...
EcC3000_featureCounts.TPM <- t(t(EcC3000_featureCounts_matrix.norm_by_length)*10**6/colSums(EcC3000_featureCounts_matrix.norm_by_length)) %>% 
  ...
  select(-c("Start", "End", "Strand", "Length", "Chr")) %>% 
  select(GeneID, Name, everything())
names(EcC3000_featureCounts.TPM) <- sub("_S[[:digit:]]+_sorted.bam", "", names(EcC3000_featureCounts.TPM))
...
Ecd10LVM_featureCounts_raw <- left_join(as.data.table(Ecd10LVM_featureCounts$counts, keep.rownames = "GeneID"),
  ...
  select(-c("Start", "End", "Strand", "Length", "Chr")) %>% 
  select(GeneID, Name, everything())
names(Ecd10LVM_featureCounts_raw) <- sub("_S[[:digit:]]+_sorted.bam", "", names(Ecd10LVM_featureCounts_raw))
...
Ecd10LVM_featureCounts.TPM <- t(t(Ecd10LVM_featureCounts_matrix.norm_by_length)*10**6/colSums(Ecd10LVM_featureCounts_matrix.norm_by_length)) %>% 
  ...
  select(-c("Start", "End", "Strand", "Length", "Chr")) %>% 
  select(GeneID, Name, everything())
names(Ecd10LVM_featureCounts.TPM) <- sub("_S[[:digit:]]+_sorted.bam", "", names(Ecd10LVM_featureCounts.TPM))
fwrite(x = Ecd10LVM_featureCounts.TPM, file = "Results/Tables/featureCountsTables/Ecd10LVM_featureCounts_TPM_normalized.tsv",
       sep = "\t", quote = F, row.names = F)
