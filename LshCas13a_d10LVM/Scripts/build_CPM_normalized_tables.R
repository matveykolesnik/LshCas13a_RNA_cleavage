...
design_table <- fread("design.tsv", sep = "\t", stringsAsFactors = F, header = T)

targeting_samples <- design_table[Exp_group == "targeting" & TAP_treatment == "untreated"]$Sample
nontargeting_samples <- design_table[Exp_group == "nontargeting" & TAP_treatment == "untreated"]$Sample

N5E_counts_table <- read.delim("Results/Tables/Merged_ends_counts/N5E_TAP_untreated_T_vs_NT.tsv.gz", 
...
fwrite(N5E_counts_table_CPM, "Results/Tables/Merged_ends_counts/N5E_TAP_untreated_T_vs_NT_CPM_normalized.tsv", 
